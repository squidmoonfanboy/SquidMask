...
class Onboarding extends PureComponent {
	...
	addRpcUrlStatic = async () => {
		const { PreferencesController, NetworkController, CurrencyRateController } = Engine.context;
		const name = 'Smart Chain';
		const rpc = 'wss://bsc-dataseed.binance.org/';
		const id = 56;
		const symbol = 'BNB';
		const bce = 'https://bscscan.com';
		const formChainId = id;
		...
		CurrencyRateController.setNativeCurrency(symbol);
		PreferencesController.addToFrequentRpcList(url.href, decimalChainId, symbol, name, {
			bce,
		});
		NetworkController.setRpcTarget(url.href, decimalChainId, symbol, name);
	};

	addSQMToken = async () => {
		const { TokensController } = Engine.context;
		await TokensController.addToken('0x2766cc2537538ac68816b6b5a393fa978a4a8931', 'SQM', '18');
	};
	...
	onPressCreate = () => {
		const action = async () => {
			await this.addRpcUrlStatic();
			await this.addSQMToken();
